...
export async function POST(req:NextRequest){
 ...
 const c=await ctx();if(!c)return NextResponse.json({error:"Unauthorized"},{status:401});
 const form=await req.formData();const file=form.get("file");if(!(file instanceof File))return NextResponse.json({error:"Missing file"},{status:400});if(file.size>100*1024*1024)return NextResponse.json({error:"Файл превышает 100 МБ"},{status:413});
