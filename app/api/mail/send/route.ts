...
export async function POST(req:NextRequest){
 ...
 const submission:any={identityId:identity.id,emailId};const holdUntilMs=scheduleAt?scheduledMs:(undoable?undoUntilMs:0);if(holdUntilMs){submission.envelope={mailFrom:{email:identity.email,parameters:{HOLDUNTIL:String(Math.floor(holdUntilMs/1000))}},rcptTo:allRecipients.map((x:any)=>({email:x.email}))}};
