...
export async function POST(req:NextRequest){
 ...
 const submission:any={identityId:identity.id,emailId};const holdSeconds=scheduleAt?delaySeconds:(undoable?undoHoldSeconds:0);if(holdSeconds>0){submission.envelope={mailFrom:{email:identity.email,parameters:{HOLDFOR:String(holdSeconds)}},rcptTo:allRecipients.map((x:any)=>({email:x.email}))}};
