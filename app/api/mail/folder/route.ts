...
export async function POST(req:NextRequest){
 ...
 const requestBody=await req.json().catch(()=>({}));const action=String(requestBody.action||""),id=String(requestBody.id||""),name=String(requestBody.name||"");if(!action)return NextResponse.json({error:"Missing action"},{status:400});if(name.length>255)return NextResponse.json({error:"Название папки слишком длинное"},{status:400});
 ...
 const jmapBody={using:["urn:ietf:params:jmap:core","urn:ietf:params:jmap:mail"],methodCalls:[["Mailbox/set",args,"m"]]};
 const r=await fetch(c.endpoint,{method:"POST",headers:c.headers,body:JSON.stringify(jmapBody),cache:"no-store"});const d=await r.json();const x=d.methodResponses?.[0];
